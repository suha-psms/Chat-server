...
private:
  ...
  //added
  bool valid(const std::string& tag);
  bool check_if_colon(std::string buff_string, size_t colon_pos); 
  bool parse_header(const char* buf, Message &msg);
  void handle_receive_error(); 
  void set_message_data(Message& msg, const std::string& tag, const std::string& payload); 
